    ...
    try {
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
            try {
                ...
                if (attempt === 3)
                    throw new Error("Failed to compile after 3 attempts.");
                // Ask LLM to fix it
        ...
        const payload = (0, deployer_1.generateDeploymentPayload)(compiled.abi, compiled.bytecode, constructorArgs);
        console.log("4. Running Security Audit...");
        const auditReport = await (0, llm_1.auditContract)(solidityCode);
        res.json({
            status: "success",
